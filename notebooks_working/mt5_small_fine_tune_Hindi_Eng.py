...
output_file = open('mt5_small_output_hindi_eng.log', 'w')
...
max_seq_len = 25
model_path = './models/mt5-hindi-eng/'
...
def format_translation_data(translation, lang_token_map, tokenizer, seq_len=128):
    ...
    input_text = translation.get('hi')
    target_text = translation.get('en')

    if input_text is None or target_text is None:
        return None

    input_token_ids = encode_input_str(
        input_text, 'en', tokenizer, seq_len, lang_token_map)
...
def evaluate_translations_bertscore(test_data, translated_test_data):
    ...
    P, R, F1 = score(translations, references, lang="en", verbose=True)
...
def evaluate_translations_bleu(test_data, translated_test_data, tokenizer):
    ...
    references = [[tokenizer.tokenize(test_data[i]['translation']['en'])] for i in range(len(test_data))]
...
# %%
fine_tune_and_save_model(model, tokenizer, small_data_set['train'], small_data_set['validation'], LANG_TOKEN_MAPPING, './mt5-small_fine_tune_hindi_eng')

# %%
save_fine_tuned_model(model, tokenizer, './mt5-small_fine_tune_hindi_eng')

# %%
model, tokenizer = load_fine_tuned_model('./mt5-small_fine_tune_hindi_eng')
...
translated_test_data = perform_translation_testing(model, tokenizer, test_data, tokenized_test_data, target_lang="en")
